...
public class DefaultInMemorySorterFactory<T> implements InMemorySorterFactory<T> {
	...
	public InMemorySorter<T> create(List<MemorySegment> sortSegments) {
		...
		if (useFixedLengthRecordSorter) {
			return new FixedLengthRecordSorter<>(typeSerializer.duplicate(), duplicateTypeComparator, sortSegments);
		} else {
			return new NormalizedKeySorter<>(typeSerializer.duplicate(), duplicateTypeComparator, sortSegments);
		}
	}
}
