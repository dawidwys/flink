...
public class RocksDBFullRestoreOperation<K> extends AbstractRocksDBRestoreOperation<K> {
    ...
    public RocksDBRestoreResult restore()
        ...
        openDB();
        List<SavepointRestoreResult> restore = savepointRestoreOperation.restore();
        for (SavepointRestoreResult savepointRestoreResult : restore) {
            applyRestoreResult(savepointRestoreResult);
        }
        return new RocksDBRestoreResult(
                this.db, defaultColumnFamilyHandle, nativeMetricMonitor, -1, null, null);
    }

    private void applyRestoreResult(SavepointRestoreResult savepointRestoreResult) throws IOException, RocksDBException {
        List<StateMetaInfoSnapshot> restoredMetaInfos = savepointRestoreResult.getStateMetaInfoSnapshots();
        List<ColumnFamilyHandle> columnFamilyHandles = restoredMetaInfos.stream()
                .map(
    ...
    private void restoreKVStateData(
            ExceptionIterator<KeyGroup> keyGroups,
            List<ColumnFamilyHandle> columnFamilies) throws IOException, RocksDBException {
        ...
        try (
            ...
            while (keyGroups.hasNext()) {
                ...
                ExceptionIterator<KeyGroupEntry> groupEntries = keyGroup.getKeyGroupEntries();
                while (groupEntries.hasNext()) {
                    // TODO we call it more times than before
                    KeyGroupEntry groupEntry = groupEntries.next();
                    ColumnFamilyHandle handle = columnFamilies.get(
                            groupEntry.getKvStateId());
